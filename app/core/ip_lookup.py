...
from app.core.func import retry
...
class QQwry:
    ...
    def load_file(self, filename: Union[str, bytes], loadindex: bool = False) -> bool:
        ...
        if len(buffer) < 8:
            logger.error("%s load failed, file only %d bytes" % (filename, len(buffer)))
            self.clear()
            return False
        ...
        if (
            ...
            or (index_end - index_begin) % 7 != 0
            or index_end + 7 > len(buffer)
        ):
        ...
        if not loadindex:
            logger.info(
                "%s %s bytes, %d segments. without index."
        ...
        logger.info(
            "%s %s bytes, %d segments. with index."
    ...
    def __get_addr(self, offset):
        if not self.data:
            raise Exception("Error: qqwry.dat not loaded yet.")
        # mode 0x01, full jump
        ...
        return c.decode("gb18030", errors="replace"), p.decode(
    ...
    def __index_search(self, ip):
        if not self.idx1 or not self.idx2 or not self.idxo:
            raise Exception("Error: qqwry.dat not loaded yet.")
...
def setup_qqwry(update: bool = False):
    ...
    if q.is_loaded():
        return
    if update or not Path(APPPATH, "qqwry.dat").exists():
        logger.info("Download qqwry.dat...")
        download_qqwry_dat(settings.QQWRY_DOWNLOAD_URL, Path(APPPATH, "qqwry.dat"))
        logger.info("Download qqwry.dat success start load qqwry.dat...")

    logger.info("Load qqwry.dat...")
    q.load_file(Path(APPPATH, "qqwry.dat").as_posix())
