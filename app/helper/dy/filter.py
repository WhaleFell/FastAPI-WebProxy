...
class JSONModel:
    ...
    def _get_attr_value(self, jsonpath_expr):
        ...
        if result:
            # If only one result, return the value directly
            return (
...
class UserPostFilter(JSONModel):
    ...
    def video_bit_rate(self):
        ...
        return [
            ...
            else [aweme[0]["bit_rate"]]  # type: ignore
            if len(aweme) == 1
            else [item["bit_rate"] for item in aweme]  # type: ignore
...
class UserLive2Filter(JSONModel):
    ...
    @property
    def create_time(self):
        return timestamp_2_str(self._get_attr_value("$.data.room.create_time"))  # type: ignore

    @property
    def finish_time(self):
        return timestamp_2_str(self._get_attr_value("$.data.room.finish_time"))  # type: ignore
